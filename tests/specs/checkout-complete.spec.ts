...
const checkoutCompleteTests = (userType: UseType) => {
  test.describe(`Checkout Complete Tests - ${userType} user`, () => {
    ...
    test("should display complete order elements", async () => {
      expect(await checkoutCompletePage.isCompleteContainerVisible()).toBeTruthy();
      expect(await checkoutCompletePage.isCompleteHeaderVisible()).toBeTruthy();
      expect(await checkoutCompletePage.isCompleteTextVisible()).toBeTruthy();
      expect(await checkoutCompletePage.isBackHomeButtonVisible()).toBeTruthy();
      expect(await checkoutCompletePage.isPonyExpressImageVisible()).toBeTruthy();
    });

    test("should display correct completion messages", async () => {
